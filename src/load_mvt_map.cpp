...
static constexpr double mapbox_zoom2scale(double zoom) {
...
static bool parse_style(mapnik::Map &map, const std::string &style_str, const std::string& base_path);
...
struct SymbolParser;
static std::string normalize_font(std::string raw_font);
...
void load_mvt_map(mapnik::Map & map, std::string const& filename, bool strict, const std::string& base_path) {
    ...
    load_mvt_map_string(map, style_str, strict, dir_path);
}

void load_mvt_map_string(mapnik::Map & map, std::string const& str, bool strict, const std::string& base_path) {
    if (!parse_style(map, str, base_path)) {
...
static void ParseTextProperties(const PropertyParser &prop_parser, mapnik::symbolizer_base& symb) {
    ...
    if (tfonts && (*tfonts).size() > 0) {
        p->defaults.format_defaults.face_name = normalize_font((*tfonts)[0]);
    }
    ...
    if (prop_parser.has_property("text-transform")) {
        LOG(ERROR) << "Cannot deal with text-transform ";
    }

    if (prop_parser.has_property("text-ignore-placement")) {
        LOG(WARNING) << "Cannot deal with text-ignore-placement";
    }

    if (prop_parser.has_property("text-halo-blur")) {
        LOG(WARNING) << "Cannot deal with halo blur! ";
    }
...
static void ParseIconProperties(const PropertyParser &prop_parser, const std::string& res_path,
    ...
    if (prop_parser.has_property("icon-text-fit")) {
        LOG(WARNING) << "Cannot deal with icon texft fit!";
    }
}
...
static std::string normalize_font(std::string raw_font)
{
    if(raw_font == "DejaVu Sans Bold" || raw_font == "DejaVuSans-Bold")
        return "DejaVu Sans Bold";
    //else if (raw_font == "calibrili") // no
    //    return "Calibrili";
    else if(raw_font == "DejaVuSans") // yes
        return "DejaVu Sans Book";
    else if(raw_font == "DejaVuSansCondensed") //yes
        return "DejaVu Sans Condensed";
    else if (raw_font == "DejaVuSans-Oblique") //yes
        return "DejaVu Sans Oblique";
    else if (raw_font == "DejaVuSansCondensed-Oblique") //yes
        return "DejaVu Sans Condensed Oblique";
    else
    {
        //LOG(WARNING) << raw_font;
        return "DejaVu Sans Bold";
    }
}
